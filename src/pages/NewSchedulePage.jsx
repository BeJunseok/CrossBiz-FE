...
import { addEvent, updateEvent, removeEventById } from "../utils/eventStore";
...
export default function NewSchedulePage() {
  ...
  const preset = state?.preset; // { dateISO, title, code, office?, fromArrow?, eventId? }
  ...
  const [office, setOffice] = useState(preset?.office ?? "");

  const [openEndTime, setOpenEndTime] = useState(false);
  const [endView, setEndView] = useState(defEnd);
  ...
  const onSave = () => {
    ...
    if (isEdit) {
      updateEvent(editingId, patch);   // ← 기존 이벤트 수정
    } else {
      addEvent(patch);                  // ← 새 이벤트 추가
    }
    nav(-1);
  };
  ...
  const resetForm = () => {
    const now = new Date();
    const sDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0);
    const eDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 1, 0);
    setArrowCode(null);
    setTitle("");
    setOffice("");
    setStart(toLocalInput(sDate));
    setEnd(toLocalInput(eDate));
    setEndView(eDate);
  };
  ...
  return (
    <div className="w-full min-h-screen bg-[#f3f3f3]">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b bg-white px-4 py-3">
        ...
        <h1 className="text-[17px] font-semibold text-gray-900">{isEdit ? "일정 수정" : "새 일정"}</h1>
      ...
      <main className="px-4 pt-4 pb-24">
        ...
        {/* 삭제 */}
        <div className="mt-3">
          <button type="button" onClick={onDelete} className="w-full rounded-xl border text-red-600 bg-white py-3 text-[15px] font-semibold active:scale-[0.99] shadow-md">
            일정 삭제
          </button>
        </div>
      </main>
